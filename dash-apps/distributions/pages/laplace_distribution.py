...
layout = html.Div([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dbc.Row(
                dbc.Col(
                    dcc.Graph(id="laplacedist-plot", mathjax=True, config=configuracoes_grafico),
                ...
                dbc.Col(
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-xmin-input", type="number", value=-6.1, autofocus=False, debounce=False,
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-xmax-input", type="number", value=6.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-ymin-input", type="number", value=-.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-ymax-input", type="number", value=.8, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                ...
                dbc.Col(
                    dmc.Tooltip(
                        ...
                        children=dmc.Switch(label="", onLabel="Sim", offLabel="Não", checked=True, size="lg", color="teal",
                                        id="laplacedist-switch", persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE),
        ...
        dbc.Col([
                ...
                dbc.Col(
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-loc-input", type="number", value=0, step=.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="laplacedist-scale-input", type="number", value=1, min=0, step=.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                    ...
                    dbc.Col(
                        dbc.Row([
                            ...
                            dbc.Col([
                                dbc.Row(
                                    dbc.Col(
                                        dbc.Input(id="laplacedist-size-input", type="number", value=20, step=1, min=0, autofocus=False, debounce=False,
                                        persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                        )
                                    ),
                                ]),
                            ...
                            dbc.Col([
                                dbc.Row(
                                    dbc.Col(
                                        dbc.Input(id="laplacedist-random-input", type="number", value=42, step=1, min=0, autofocus=False, debounce=False,
                                        persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                        )
                                    ),
                                ]),
            ...
            ], id="laplacedist-param-div"),

            # alert
            dbc.Row(
                dbc.Col(
                    id="laplacedist-alert"
                )
            )
        ])
    ], justify="center",),
    ...
    dbc.Row(
        dbc.Col(
            html.Ul([
                html.Li([
                    html.Span("Wikipedia contributors. Laplace distribution. In Wikipedia, The Free Encyclopedia (2023), "),
                    html.A("en.wikipedia.org/wiki/Laplace_distribution", href="https://en.wikipedia.org/w/index.php?title=Laplace_distribution&oldid=1142109776"),
                    html.Span(". Acesso em 09/03/2023.")
                ]),
                html.Li([
                    ...
                    html.Span(", "),
                    html.A("scipy.stats.laplace", href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.laplace.html"),
...
@callback(
    Output(component_id="laplacedist-param-div", component_property="style"),
    Input(component_id="laplacedist-switch", component_property="checked"),
)
...
@callback(
    Output(component_id="laplacedist-plot", component_property="figure"),
    Output(component_id="laplacedist-alert", component_property="children"),
    Input(component_id="laplacedist-xmin-input", component_property="value"),
    Input(component_id="laplacedist-xmax-input", component_property="value"),
    Input(component_id="laplacedist-ymin-input", component_property="value"),
    Input(component_id="laplacedist-ymax-input", component_property="value"),
    Input(component_id="laplacedist-loc-input", component_property="value"),
    Input(component_id="laplacedist-scale-input", component_property="value"),
    Input(component_id="laplacedist-size-input", component_property="value"),
    Input(component_id="laplacedist-random-input", component_property="value"),
    Input(component_id="laplacedist-switch", component_property="checked"),
)
def update_plot(x_min, x_max, y_min, y_max, loc, scale, size, random, switch):
    ...
    if not isinstance(scale, numbers.Number):
        text = "O parâmetro de escala (b) deve ser um valor numérico!"
        dica = html.Div([
            html.P("O separador de casas decimais é definido pela sua região (seu país)."),
        ])
        alert = distfuncs.make_alert(text, dica)
    ...
    if scale <= 0:
        text = "O parâmetro de escala (b) deve ser maior do que zero (0)"
        alert = distfuncs.make_alert(text)
        return no_update, alert
    ...
    x = np.linspace(laplace.ppf(0.0001, loc=loc, scale=scale), laplace.ppf(0.9999, loc=loc, scale=scale), 1000)

    df_distribution = pd.DataFrame({
        "x": x,
        "Densidade": laplace.pdf(x, loc=loc, scale=scale)
    })
    ...
    fig['data'][0]['name'] = 'Dist. Laplace'
    fig['data'][0]['line']['color']='black'

    if switch:
        x_data = laplace.rvs(loc=loc, scale=scale, size=size, random_state=random)
        df_data = pd.DataFrame({
            "x": x_data,
            "Dados":  laplace.pdf(x_data, loc=loc, scale=scale)
        })
        # adicionando a reta
        fig.add_trace(
            go.Scatter(
