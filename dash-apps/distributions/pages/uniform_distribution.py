                    ...
                    dbc.Row(
                        dbc.Col([
                            html.Ul([
                                html.Li(
                                    dcc.Markdown(
                                        "$A$ é o parâmetro de localização;",
                                        mathjax=True
                                        )
                                ),
                                html.Li(
                                    dcc.Markdown(
                                        "$B-A$ é o parâmetro de escala;",
                                        mathjax=True
                                        )
                                ),
                            ])
        ...
        dbc.Col(
            html.Ul([
                html.Li([
                    ...
                    html.Span(", "),
                    html.A("Uniform Distribution", href="https://www.itl.nist.gov/div898/handbook/eda/section3/eda3662.htm"),
                ...
                html.Li([
                    ...
                    html.Span(", "),
                    html.A("scipy.stats.uniform", href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.uniform.html"),
                    html.Span(". Acesso em 02/02/2023.")
                ])
            ])
        )
...
def update_plot(x_min, x_max, y_min, y_max, loc, scale, size, random, switch):
    ...
    if not isinstance(loc, numbers.Number):
        text = "O parâmetro de localização ser um valor numérico!"
        dica = html.Div([
            html.P("O separador de casas decimais é definido pela sua região (seu país)."),
        ])
        alert = distfuncs.make_alert(text, dica)
    ...
    if not isinstance(scale, numbers.Number):
        text = "O parâmetro de escala (B-A) deve ser um valor numérico!"
        dica = html.Div([
            html.P("O separador de casas decimais é definido pela sua região (seu país)."),
        ])
        alert = distfuncs.make_alert(text, dica)
    ...
    if scale <= 0:
        text = "O parâmetro de escala (B-A) deve ser maior do que zero (0)"
        alert = distfuncs.make_alert(text)
        return no_update, alert
