...
layout = html.Div([
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            dbc.Row(
                dbc.Col(
                    dcc.Graph(id="gammadist-plot", mathjax=True, config=configuracoes_grafico),
                ...
                dbc.Col(
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-xmin-input", type="number", value=-.5, autofocus=False, debounce=False,
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-xmax-input", type="number", value=3.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-ymin-input", type="number", value=-.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-ymax-input", type="number", value=1.25, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                ...
                dbc.Col(
                    dmc.Tooltip(
                        ...
                        children=dmc.Switch(label="", onLabel="Sim", offLabel="Não", checked=True, size="lg", color="teal",
                                        id="gammadist-switch", persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE),
        ...
        dbc.Col([
            ...
            dbc.Row([
                dbc.Col(
                    html.H4(
                        dcc.Markdown(
                            "$f \\left(x \\right) = \\frac{\\left( \\frac{x - \\mu}{\\beta} \\right)^{\\gamma - 1}\\exp{(-\\frac{x-\\mu}{\\beta}})} {\\beta\\Gamma(\\gamma)}$",
                            mathjax=True
                            )
                    ...
                    dbc.Row(
                        dbc.Col([
                            html.Ul([
                                ...
                                html.Li(
                                    dcc.Markdown(
                                        "$\\gamma$ é o parâmetro de forma;",
                                        mathjax=True
                                        )
                                ),
                                html.Li(
                                    dcc.Markdown(
                                        "$\\Gamma$ é $\\Gamma(\\gamma) = \\int_{0}^{\\infty} {t^{\\gamma-1}e^{-t}dt}$;",
                                        mathjax=True
                                        )
                                ),
                            ])
                ...
                dbc.Col(
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-loc-input", type="number", value=0, step=.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-scale-input", type="number", value=1, min=0, step=.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                        ...
                        dbc.Col([
                            dbc.Row(
                                dbc.Col(
                                    dbc.Input(id="gammadist-shape-input", type="number", value=1, min=0, step=.1, autofocus=False, debounce=False,
                                    persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                    )
                                ),
                            ]),
                    ...
                    dbc.Col(
                        dbc.Row([
                            ...
                            dbc.Col([
                                dbc.Row(
                                    dbc.Col(
                                        dbc.Input(id="gammadist-size-input", type="number", value=20, step=1, min=0, autofocus=False, debounce=False,
                                        persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                        )
                                    ),
                                ]),
                            ...
                            dbc.Col([
                                dbc.Row(
                                    dbc.Col(
                                        dbc.Input(id="gammadist-random-input", type="number", value=42, step=1, min=0, autofocus=False, debounce=False,
                                        persistence_type=PERSISTENCE_TYPE, persistence=PERSISTENCE)
                                        )
                                    ),
                                ]),
        ...
        dbc.Col(
            html.Ul([
                html.Li([
                    ...
                    html.Span(", "),
                    html.A("Gamma Distribution", href="https://www.itl.nist.gov/div898/handbook/eda/section3/eda366b.htm"),
                ...
                html.Li([
                    ...
                    html.Span(", "),
                    html.A("scipy.stats.gamma", href="https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.gamma.html"),
                    html.Span(". Acesso em 02/02/2023.")
                ])
            ])
        )
...
@callback(
    Output(component_id="gammadist-param-div", component_property="style"),
    Input(component_id="gammadist-switch", component_property="checked"),
)
...
@callback(
    Output(component_id="gammadist-plot", component_property="figure"),
    Output(component_id="gammadist-alert", component_property="children"),
    Input(component_id="gammadist-xmin-input", component_property="value"),
    Input(component_id="gammadist-xmax-input", component_property="value"),
    Input(component_id="gammadist-ymin-input", component_property="value"),
    Input(component_id="gammadist-ymax-input", component_property="value"),
    Input(component_id="gammadist-loc-input", component_property="value"),
    Input(component_id="gammadist-scale-input", component_property="value"),
    Input(component_id="gammadist-shape-input", component_property="value"),
    Input(component_id="gammadist-size-input", component_property="value"),
    Input(component_id="gammadist-random-input", component_property="value"),
    Input(component_id="gammadist-switch", component_property="checked"),
)
def update_plot(x_min, x_max, y_min, y_max, loc, scale, shape, size, random, switch):
    ...
    if not isinstance(shape, numbers.Number):
        text = "O parâmetro de forma (γ) deve ser um valor numérico!"
        dica = html.Div([
            html.P("O separador de casas decimais é definido pela sua região (seu país)."),
        ])
        alert = distfuncs.make_alert(text, dica)
    ...
    if shape <= 0:
        text = "O parâmetro de forma (γ) deve ser maior do que zero (0)"
        alert = distfuncs.make_alert(text)
        return no_update, alert
    ...
    x = np.linspace(gamma.ppf(0.0001, a=shape, loc=loc, scale=scale), gamma.ppf(0.9999, a=shape, loc=loc, scale=scale), 1000)

    df_distribution = pd.DataFrame({
        "x": x,
        "Densidade": gamma.pdf(x, a=shape, loc=loc, scale=scale)
    })
    ...
    fig['data'][0]['name'] = 'Dist. Gamma'
    fig['data'][0]['line']['color']='black'

    if switch:
        x_data = gamma.rvs(a=shape, loc=loc, scale=scale, size=size, random_state=random)
        df_data = pd.DataFrame({
            "x": x_data,
            "Dados":  gamma.pdf(x_data, a=shape, loc=loc, scale=scale)
        })
        # adicionando a reta
        fig.add_trace(
            go.Scatter(
